param(
    ...
    [string]$zipPath = 'src/services/w2-intake/deploy.zip'
)

Write-Host "Packaging and deploying W-2 intake function app code for resource group $resourceGroupName"

if (-not (Test-Path 'src/services/w2-intake')) {
    throw "Function app source folder src/services/w2-intake not found."
}
...
Compress-Archive -Path 'src/services/w2-intake/*' -DestinationPath $zipPath -Force
