...
resource uploadW2OperationPolicy 'Microsoft.ApiManagement/service/apis/operations/policies@2022-08-01' = {
  ...
  properties: {
    format: 'rawxml'
    value: '<policies><inbound><base /><cors allow-credentials="false"><allowed-origins><origin>${portalWebOrigin}</origin><origin>http://localhost:5173</origin></allowed-origins><allowed-methods><method>POST</method><method>OPTIONS</method></allowed-methods><allowed-headers><header>Authorization</header><header>Content-Type</header><header>correlation-id</header></allowed-headers><expose-headers><header>correlation-id</header></expose-headers></cors>${portalJwtPolicy}<set-header name="x-functions-key" exists-action="override"><value>{{w2-intake-function-key}}</value></set-header><set-header name="x-demo-portal" exists-action="override"><value>w2-upload</value></set-header></inbound><backend><base /></backend><outbound><base /></outbound><on-error><base /></on-error></policies>'
  }
}
